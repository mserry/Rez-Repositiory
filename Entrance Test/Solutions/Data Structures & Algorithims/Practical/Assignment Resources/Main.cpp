...
void TestTechTree()
{
    ...
    // individual test
    auto runSingleTest = [&](int testNum, int goalIndex) -> bool
    {
        techTree.FindBestPath(goalIndex, path);
    ...
    expectedPath = { "Being Nice to Books", "Being Alone", "Drug Experimentation", "Inhumane Human Experiments" };
    runSingleTest(3, 12);

    // Test #4: Cost to initial tech
    expectedPath = { "Being a Dick to People" };
    runSingleTest(4, 4);
